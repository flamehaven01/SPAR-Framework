...
def build_layer_a(
    *,
    ...
    defaults = get_layer_a_defaults()["beta_tolerances"]
    beta_tol_g = float(params.get("beta_tol_G", defaults["beta_G"]))
    beta_tol_b = float(params.get("beta_tol_B", defaults["beta_B"]))
    beta_tol_phi = float(params.get("beta_tol_Phi", defaults["beta_Phi"]))
